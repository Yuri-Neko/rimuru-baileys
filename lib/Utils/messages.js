...
async function prepareStickerPackMessageOri(stickerPack, options) {
	...
	try {
		const trayStream = await getStream(stickers[0].data)
		const trayBuffer = await toBuffer(trayStream.stream)
		let thumbnailBuffer

		if (lib.sharp) {
			thumbnailBuffer = await lib.sharp.default(trayBuffer)
				.resize(252, 252)
				.jpeg()
				.toBuffer()
		} else if (lib.jimp) {
		...
		const thumbUploadResult = await options.upload(
			thumbUpload.encFilePath,
			{
